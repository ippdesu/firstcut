...
| AI 推理 | `ort`（onnxruntime-rs） | 跑人脸检测 + NIMA；GPU 走 DirectML/CUDA provider |
| 人脸检测 | YuNet ONNX 模型（OpenCV Zoo，~350KB） | 小、快、准，适合构图评分 |
| 美学评分 | NIMA（MobileNet 变体）转 ONNX | 输出 1-10 分布 |
...
1. **清晰度/合焦**：下采样→灰度→拉普拉斯/梯度方差（Tenengrad）；结合 EXIF 光圈判断景深（大光圈浅景深放宽阈值，避免误杀焦外风景）。
2. **曝光**：直方图统计过曝（>250）与欠曝（<10）像素比例 + 亮度偏离中间调程度。
3. **噪点**：EXIF ISO + 暗部区域标准差（最暗 5% 像素块方差），ISO 越高容差越大。
4. **构图**：YuNet 检测人脸 → 人脸数量/大小/位置；三分法评分（主体偏离中心惩罚）；边缘裁切检测。
5. **美学**：NIMA 输出 1-10 分布取均值。
6. **连拍去重**：按 `DateTimeOriginal` 时间戳聚类（间隔 <2s 为一组）→ 组内 pHash 相似度 → 组内排序保留 top-K（默认 1-2）并标记"组内第 N 名"。

**汇总**：可配置权重（提供默认推荐值），总分 0-100 + 6 个子分全部进 CSV，便于人工复核与后续调参。
...
- **M0 骨架**：CLI + 目录扫描 + EXIF + CSV 输出（空跑通，验证管道）
- **M1 像素指标**：清晰度/曝光/噪点 + 加权总分（纯算法，无 AI 依赖）
- **M2 连拍去重**：时间聚类 + pHash 组内排序
- **M3 AI 接入**：模型下载脚本 + NIMA + 人脸检测 → 构图/美学维度
- **M4 输出完善**：XMP 星级写出 + SQLite 增量缓存
- **M5 调参与验证**：用真实照片跑通、对照人工判断调权重、性能优化

## 8. 风险与开放问题

- **NIMA 转 ONNX**：现成 ONNX 权重不官方，需要按教程转换（有成熟路径）；备选 MUSIQ 或先只依赖像素指标，AI 维度后置。
